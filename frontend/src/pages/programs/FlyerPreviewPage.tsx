...
function formatTimeES(timeStr: string): string {
  if (!timeStr) return ''
  // Normalizar a HH:MM (sin segundos) y convertir a formato 12h
  const parts = timeStr.split(':')
  const h = parseInt(parts[0], 10)
  const m = (parts[1] || '00').padStart(2, '0')
  
  // Determinar AM/PM basado en la hora (0-23)
  const calculatedAmpm = h >= 12 ? 'PM' : 'AM'
  const h12 = h > 12 ? h - 12 : h === 0 ? 12 : h
  
  return `${h12}:${m} ${calculatedAmpm}`
}
...
const FlyerPreviewPage = () => {
  ...
  const exportPDFLocal = useCallback(() => {
    ...
    y += 6
    const timeStr = formatTimeES(form.timeInput)
    setTxt(...GRAY5)
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    ...
    // GUARDAR
    const fname = `programa-${form.worshipType.replace(/\s/g, '-')}-${form.dateInput || 'sin-fecha'}.pdf`
    doc.save(fname)
            ...
            <div style={{
              ...
              <div style={styles.flyerContainer}>
                ...
                <div style={styles.dateRow}>
                  ...
                  <div style={styles.flyerTime}>{formatTimeES(form.timeInput)}</div>
                </div>
